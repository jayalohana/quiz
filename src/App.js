...
function App() {
  ...
  const endGameScreen = () => (
    ...
    <div className="mb-auto p-6 header-bg">
      ...
      <p>You can keep playing to stand to win the cash price of $20 000. Answer the next 5 questions.</p>
      <p>What will you do?</p>
      <div className="flex p-3 text-white">
        <div className="py-2 px-5 cursor-pointer btn-primary rounded-xl mr-3">Cash Out</div>
        <div className="py-2 px-5 cursor-pointer bg-green-500 rounded-xl">Keep playing</div>
      </div>
    </div>
    </> : 
      <div className="my-auto p-6 header-bg">
        <h2 className="font-bold text-lg">You Did Not Advance</h2>
        <p>It seems like you are lacking in general knowledge... Your welcome to play again</p>
        <div className="flex p-3 text-white">
          <div className="py-2 px-5 cursor-pointer bg-green-500 rounded-xl mr-3" onClick={ () => setCurrentGamePlaying(current => current +1)}>Play Again</div>
        </div>
      </div>
  )
  ...
  return (
    <div className="container h-screen items flex flex-col">
      ...
      <img src={quizLogo} className=" w-1/3 mx-auto mb-4" />
      <div className="text-xl text-center btn-primary">
        ...
        {currentGame[currentQuestion]?.content.map( (answer, index) => (
            <div key={index} className="container btn-container items-center flex border border-gray-700 mb-2 rounded-3xl cursor-pointer" onClick={(el) =>{ updateScore(index, el) } }>
